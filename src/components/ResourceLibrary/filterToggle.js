    ...
    return (
      <div className="md:w-1/3 mb-10 lg:w-1/4">
        <div className="md:hidden m-auto max-w-xs relative">
          <button className="bg-green-mid hover:bg-grey-dark focus:bg-grey-dark text-white text-20 text-left font-header font-extrabold px-4 py-3half w-full" onClick={this.toggle.bind(this)} aria-controls="resource-filter-list" aria-expanded={String(shown)}>
            {String(shown) === 'true' ? 'Close Filters:' : 'Filters Results:'}
            <FilterIcon className="filter-icon" />
          </button>
        </div>
        <div className="hidden relative md:block bg-green-mid text-white text-20 font-header font-extrabold px-4 py-8">
          Filter Results:
          <FilterIcon className="filter-icon" />
        </div>
